...
export async function GET(req: Request) {
  ...
  try {
    const hotNews = await getNewsHot(page, pageSize);
    const list = hotNews.list ?? [];

    // Sentiment analysis
    const bullish = list.filter((n) => {
      const t = (n.title + n.content).toLowerCase();
      return t.includes("surge") || t.includes("rally") || t.includes("bull") || t.includes("breakout") ||
        t.includes("inflow") || t.includes("accumul") || t.includes("upgrade");
    }).length;

    const bearish = list.filter((n) => {
      const t = (n.title + n.content).toLowerCase();
      return t.includes("crash") || t.includes("dump") || t.includes("bear") || t.includes("outflow") ||
